...
class Event:
    ...
    def handle(self, sim_manager, state: State):
        ...
        :param sim_manager: fabricatio_rl.env_control.SimulationManager
        :type state: fabricatio_rl.jssp_env.State
        """
...
class TransportArrivalEvent(Event):
    ...
    def handle(self, sim_manager, state: State):
        ...
        if decision_needed:
            state.machines.update_utilization(self.occurence_time)

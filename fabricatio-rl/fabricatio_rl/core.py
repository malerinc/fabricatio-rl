...
from fabricatio_rl.core_logger import SchedulingLogger
from fabricatio_rl.core_management import SimulationManager
from fabricatio_rl.core_state import State
from fabricatio_rl.env_utils import UndefinedSimulationModeError, \
    faster_deepcopy, IllegalAction
from fabricatio_rl.interface_input import Input


class Core:
    def __init__(self, parameters: Input, ra=True, sa=True, system_time=0):
        self.__end = False
        self.__rou_autoplay = ra
        self.__seq_autoplay = sa
        self.__skipped_seq_actions = deque([])
        self.__skipped_rou_actions = deque([])
        ...
        self.__is_deterministic = False
        # when making the environment deterministic, reinitialize everythin

    def __deepcopy__(self, memo):
        return faster_deepcopy(self, memo)

    def set_seq_autoplay(self, val: bool):
        self.__seq_autoplay = val

    def set_rou_autoplay(self, val: bool):
        self.__rou_autoplay = val

    # <editor-fold desc="Properties">
    @property
    def seq_autoplay(self):
        return self.__seq_autoplay

    @property
    def rou_autoplay(self):
        return self.__rou_autoplay

    @property
    def skipped_seq_actions(self):
        return self.__skipped_seq_actions

    @skipped_seq_actions.setter
    def skipped_seq_actions(self, val):
        assert val == []
        self.__skipped_seq_actions = val

    @property
    def skipped_rou_actions(self):
        return self.__skipped_rou_actions

    @skipped_rou_actions.setter
    def skipped_rou_actions(self, val):
        assert val == []
        self.__skipped_rou_actions = val
    # </editor-fold desc="Properties">
    ...
    def make_deterministic(self, wip_only=True):
        ...
        if len(self.sim_manager.event_heap.event_heap) > 0:
            try:
        ...
        # 2. purge the system job queue
        if wip_only:
            self.state.purge_job_queue()
        ...
        self.state.matrices.op_perturbations = np.ones((n_jobs, n_ops))
    ...
    def _get_legal_machine_assignment_actions(self):
        """
        Wait should always be at the end!s
        :return:
        """
        legal_actions = []
    ...
    def _trigger_events(self):
        ...
        while self.sim_manager.event_heap.events_available():
            ...
            try:
                assert event.occurence_time >= self.state.system_time
            except AssertionError:
                print('herehere!')
    ...
    def test_no_wait_schedulling(self):
        # TODO: add following test to tests :)
        #  The following invariant holds if scheduling waits are not allowed
        try:
            if self.state.system_time != 0:
                ops_completed = self.state.trackers.n_completed_ops
                ops_running = self.state.trackers.n_running_ops
                ops_decided = ops_completed + ops_running
                assert self.state.n_sequencing_steps == ops_decided
        except AssertionError:
            print("check it :)")
    ...
    def step(self, action: int) -> (State, bool):
        ...
        if self.state.in_scheduling_mode():
            ...
            self.state.n_sequencing_steps += 1
            # test_no_wait_schedulling
        elif self.state.in_postbuffer_routing_mode():
        ...
        self.state.trackers.update_on_step_end(
            self.state.legal_actions, self.wait_legal(),
            self.state.in_scheduling_mode(), self.state.system_time)
        if self.__seq_autoplay and self.state.scheduling_mode == 0:
            self.play_on()
        elif self.__rou_autoplay and self.state.scheduling_mode == 1:
            self.play_on()
    ...
    def play_on(self):
        if ((len(self.state.legal_actions) == 2 and self.wait_legal()) or
                len(self.state.legal_actions) == 1):
            act = self.state.legal_actions[0]
            if self.state.in_scheduling_mode():
                self.__skipped_seq_actions.append(
                    (act, self.state.current_machine_nr))
            else:
                self.__skipped_rou_actions.append(act)
            self.step(act)
